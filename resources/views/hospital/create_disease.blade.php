...
<div class="container mt-4">
    <a href="{{ url('/disease') }}" class="btn btn-primary my-4">Back</a>
        @if (session('status'))
        <div class="alert alert-success"> {{ session('status') }}</div>
        @endif
        <form method="post" action="{{ url('disease') }}">
            @csrf
    <h1 class="mb-4">Create Disease Availabilities</h1>
        <div class="form-row">
            <div class="form-group col-md-6">
                <label for="title">Disease Name</label>
                <input type="text" class="form-control" id="title" name="disease_name" required>
            </div>
        <button type="submit" class="btn btn-primary my-4">Add Disease</button>
    </form>
